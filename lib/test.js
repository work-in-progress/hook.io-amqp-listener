(function() {
  var amqp, connection, recvCount;
  amqp = require("amqp");
  connection = amqp.createConnection({
    url: "amqp://guest:guest@localhost:5672/scottyapp_dev"
  });
  recvCount = 0;
  connection.addListener("ready", function() {
    var exchange1, exchange2, q1;
    ...
    exchange2.destroy();
    return q1 = connection.queue("node-q1", function() {
      var q2;
      return q2 = connection.queue("node-q2", function() {
        q1.bind(exchange1, "node-consumer-1");
        return q1.on("queueBindOk", function() {
          q2.bind(exchange1, "node-consumer-2");
          return q2.on("queueBindOk", function() {
            console.log(Object.keys(connection.queues).length);
            q1.on("basicConsumeOk", function() {
              exchange1.publish("node-consumer-1", "foo");
              exchange1.publish("node-consumer-1", "foo");
              return exchange1.publish("node-consumer-2", "foo");
            });
            ...
            return q2.subscribe(function(m, headers, deliveryInfo) {
